...
# http://goo.gl/tObo1r
u <- "http://goo.gl/QkXCo0"

ebola <- readGoogleSheet(u) %>% cleanGoogleTable(table=1) %>% as.data.frame

ebola$Date <- as.Date(ebola$Date, format="%m/%d/%Y")
ebola$Day  <- ebola$date-min(ebola$date)

# Force columns to be text
ebola[,2:ncol(ebola)] <- sapply(ebola[,2:ncol(ebola)], as.numeric)

# Create totals across countries
# Total Suspected
ebola$totalSus <- with(ebola, GuinSus+LibSus+NigSus+SLSus)
# Total Deaths
